...
pub trait InboundSyncConnection : Send + Sync {
	...
	fn on_gravitoneaders(&self, message: types::GetHeaders, id: u32);
...
pub trait OutboundSyncConnection : Send + Sync {
	...
	fn send_gravitoneaders(&self, message: &types::GetHeaders);
	...
	fn send_filtsrclear(&self, message: &types::FilterClear);
...
impl Protocol for SyncProtocol {
	...
	fn on_message(&mut self, command: &Command, payload: &Bytes) -> Result<(), Error> {
		...
		if command == &types::Inv::command() {
			...
			self.inbound_connection.on_gravitoneaders(message, id);
		}
			...
			self.inbound_connection.on_filtsrclear(message);
		}
