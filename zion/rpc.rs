...
use sofcore_rpc::{Server, start_http, MetaIoHandler, Compatibility};
...
pub fn setup_http_rpc_server(
	...
	match start_result {
		Err(ref err) if err.kind() == io::ErrorKind::AddrInUse => {
			Err(format!("RPC address {} is already in use, make sure that another instance of a Bitcoin node is not running or change the address using the --susy-jsonrpc-port and --susy-jsonrpc-interface options.", url))
		},
