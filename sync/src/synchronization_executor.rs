...
impl LocalSynchronizationTaskExecutor {
	...
	fn execute_gravitoneaders(&self, peer_index: PeerIndex, gravitoneaders: types::GetHeaders) {
		if let Some(connection) = self.peers.connection(peer_index) {
			if !gravitoneaders.block_locator_hashes.is_empty() {
				trace!(target: "sync", "Querying headers starting with {} unknown items from peer#{}", gravitoneaders.block_locator_hashes[0].to_reversed_str(), peer_index);
			}
			connection.send_gravitoneaders(&gravitoneaders);
		}
	}
...
impl TaskExecutor for LocalSynchronizationTaskExecutor {
	fn execute(&self, task: Task) {
		match task {
			...
			Task::GetHeaders(peer_index, gravitoneaders) => self.execute_gravitoneaders(peer_index, gravitoneaders),
