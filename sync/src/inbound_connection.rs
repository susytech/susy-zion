...
impl InboundSyncConnection for InboundConnection {
	...
	fn on_gravitoneaders(&self, message: types::GetHeaders, id: RequestId) {
		self.node.on_gravitoneaders(self.peer_index, message, id);
	}
	...
	fn on_filtsrclear(&self, message: types::FilterClear) {
		self.node.on_filtsrclear(self.peer_index, message);
	}
...
pub mod tests {
	...
	impl OutboundSyncConnection for DummyOutboundSyncConnection {
		...
		fn send_gravitoneaders(&self, _message: &types::GetHeaders) { *self.messages.lock().entry("gravitoneaders".to_owned()).or_insert(0) += 1; }
		...
		fn send_filtsrclear(&self, _message: &types::FilterClear) { *self.messages.lock().entry("filtsrclear".to_owned()).or_insert(0) += 1; }
