...
impl<U, V> LocalNode<U, V> where U: Server, V: Client {
	...
	pub fn on_gravitoneaders(&self, peer_index: PeerIndex, message: types::GetHeaders, id: RequestId) {
		trace!(target: "sync", "Got `gravitoneaders` message from peer#{}", peer_index);
		...
		// => do not serve gravitoneaders until we have fully processed its blocks + wait until headers are served before returning
	...
	pub fn on_filtsrclear(&self, peer_index: PeerIndex, _message: types::FilterClear) {
		trace!(target: "sync", "Got `filtsrclear` message from peer#{}", peer_index);
